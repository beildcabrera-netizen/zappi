...
const iconMap = {
    ...
    Truck: 'M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10m10 0h3m-3 0a2 2 0 11-4 0m4 0a2 2 0 104 0m0 0h2a1 1 0 001-1v-5.586a1 1 0 00-.293-.707l-2-2A1 1 0 0015.414 7H13m-9 9h2',
...
const visibleMenu = computed(() => {
    return props.menu.filter(item => {
        ...
        return item.roles.includes(props.rol)
    })
})
</script>

<template>
    <aside class="flex h-screen w-64 flex-col overflow-y-auto border-r border-gray-200 bg-white">
        <div class="flex h-16 items-center gap-3 border-b border-gray-200 px-6">
            <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-600 text-white text-sm font-bold">F</div>
            <span class="text-lg font-bold text-gray-900">Farmacia POS</span>
        </div>
        <nav class="flex-1 space-y-1 px-3 py-4">
            ...
            >
                <svg class="h-5 w-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :d="iconMap[item.icon]" />
                </svg>
                {{ item.name }}
            </Link>
        </nav>
